...
object ClojurePlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    lazy val clojureVersion = SettingKey[String](
      "clojure-version",
      "version of clojure to use for building"
    )
    lazy val clojureSource =
      SettingKey[File]("clojure-source", "clojure source directory")
    lazy val clojureCompile = TaskKey[Unit](
      "clojure-compile",
      "runs clojure compilation, occurs before normal compilation"
    )
  }
  ...
  override lazy val projectSettings = Seq(
    ...
    unmanagedResourceDirectories in Compile += {
      (clojureSource in Compile).value
    },
    clojureCompile in Compile := {
      ...
      val classpath: Seq[File] = update.value.select(
        configurationFilter(name = "*")
      ) ++
      ...
      def clojureClazz(file: File): File = {
        ...
        new File(
          destinationDirectory.getAbsolutePath + path.substring(
            sourceDirectory.getAbsolutePath().length(),
            path.length() - ".clj".length()
          ) + ".class"
        )
      }
    ...
    compile in Compile := {
      ...
      (clojureCompile in Compile).value
      compileAnalysis
    }
